...
if (!$.Controller) {
    window.location.href =(_PORTALROOT_GLOBAL||'')+ '/index.html#/platform/login';
}
$.ajax({
...
var TreeToolBar = {
    ...
    //规则目录
    "RuleFolder": [RuleFolder, CreateBizRuleTable, ImportBizRule, BizDelete, RefreshTree],
    //规则实例的工具，添加决策表
    ...
    //业务服务目录
    "ServiceFolder": [ServiceFolder, BizService, ImportBizService, BizDelete, RefreshTree],
    //流程包
    "BizWorkflowPackage": [BizSheet, BizWorkflow, $.extend(false, { NodeType: "BizWorkflowPackage" }, BizDelete)
    ...
    //报表目录
    "ReportTemplateFolder": licenseInfo.BPA ? [ReportTemplateFolder, ReportTemplate_Summary, ReportTemplate_Cross, BizDelete, RefreshTree] : [],
...
function DoBtnActon(btnObj, treeData) {
    switch (btnObj.id) {
        ...
        case ReportTemplateFolder.id:
            ...
            break;
        case BizDelete.id:
            var msg = "<span style='color:red;'>删除主数据将导致引用失效，确认删除？<span>";
            var bizNodeType = $(btnObj).attr("NodeType");
            if (bizNodeType == "BizWorkflowPackage") {
                msg = "<span style='color:red;'>删除流程包会将相关的流程数据一并删除，确认删除？<span>";
            }
        ...
        case BizSchemaAssociation.id:
            ...
            break;
        case BizLocker.id:
            $.ligerDialog.confirm('确定锁定？', function (result) {
                if (result) {
                    $.ajax({
            ...
            break;
        case BizUnLock.id:
            $.ligerDialog.confirm('确定解锁？', function (result) {
                if (result) {
                    $.ajax({
