    ...
    $.MvcSheetUI.Controls.SheetGridView.Inherit($.MvcSheetUI.IControl, {
        ...
        SetVisible: function () {
            ...
            if (!this.DisplaySummary || this.L == $.MvcSheetUI.LogicType.BizObject) {       //汇总行
                ...
                if (!doSummary) { $(this.Summary).css("display", "none"); }
            }
        ...
        SetValue: function (dataSource) {
            // 初始化默认值 是否分页加载数据
            // HuangJie 这里应该把SetValue当成一个工厂类，根据不同属性，执行不同的方法
            if (this.VirtualPaging) {
        ...
        _RenderVirtualPagingContainer: function () {
            ...
            var tableTitleDom = $("<table class='SheetGridView'></table>").css("max-width", "none").css("width", dataTableWidth);
        ...
        RenderMobile: function () {
            ...
            //主容器 .panel-body
            var panelContainer = $(this.Element).closest("div.panel-body");
            this.Element = $("<div>").attr("data-datafield", parentDatafield).addClass("SheetGridView");
            this.Element.appendTo(panelContainer);
            var txt = $(this.Element).parent().attr("data-title") || this.DataItem.N;
            //区分关联关系
            var length = 0;
            ...
            txt = txt + "(共 " + length + " 条纪录)";
            this.title = $("<div>").addClass("item item-title").text(txt);
            ...
            var that = this;
            //绑定添加事件——add模式
            $($(this.Element).find(".addrow")).unbind('click.add').bind('click.add', [this], function (e) {
                e.data[0]._AddMobileRow();
                MobileIndexChange(that.currentIndex, 'add');
            });
            //绑定复制事件——insert模式
            $(this.copybtn.find("a")).unbind('click.copy').bind('click.copy', [this], function (e) {
            ...
            //绑定添加事件——insert模式
            $(this.addbtn.find("a")).unbind('click.add').bind('click.add', [this], function (e) {
                e.data[0]._InsertMobileRow.apply(e.data[0], [$(this), false]);
                MobileIndexChange(that.currentIndex, 'insert');
            });
            //绑定删除事件
            $(this.delebtn.find("a")).unbind('click.delete').bind('click.delete', [this], function (e) {
            ...
            var MobileIndexChange = function (index, type) {
                ...
                if (type == "delete") {
                    ...
                    var rows = $(that.Element).find("div[data-row]");
        ...
        MobileIndexButtonClick: function (element) {
            ...
            var indexEle = $(element).find("ion-scroll");
            $($(indexEle[1]).find("button")[this.currentIndex]).removeClass("button-blue");
            $($(indexEle[1]).find("button")[index]).addClass("button-blue");
        ...
        ShowMobileData: function (showIndex) {
            ...
            // 偏移量计算 left + (ele.Width+ ele.marginRigth)*index
            var marginLeft = parseInt($($(indexEle[1]).find("button")[showIndex]).parent().css('marginLeft'));
            var offsetWidth = $(indexEle[1]).find("button")[showIndex].offsetWidth;
            var marginRight = parseInt($($(indexEle[1]).find("button")[showIndex]).css('marginRight'));
            delegate.scrollTo((marginLeft + (offsetWidth + marginRight) * showIndex) - 120, 0, true);
            $($(indexEle[1]).find("button")[this.currentIndex]).removeClass("button-blue");
        ...
        _AddMobileRow: function (e) {
            ...
            */
            ...
            //this.MobileRows.push(newRow);
            this.MobileRows.splice(RowIndex - 1, 0, newRow);
            //添加行的时候，重新初始化Rule
            if ($.MvcSheetUI.MvcRuntime) $.MvcSheetUI.MvcRuntime.init();
        },
        ...
        GetChildrenTd: function (tds) {
            ...
            var td = $("<div></div>").addClass("list").attr("type", "item-text-wrap");
            var datafield = $(control).attr("data-field");
            $(control).removeAttr("data-field").attr("data-datafield", datafield);
        ...
        _SetVirtualPagingContainerHeight: function () {
            ...
            dataContainerDivHeight = dataTableHeight > dataContainerDivHeight ? dataContainerDivHeight : dataTableHeight;

            $(dataContainerDiv).css("height", dataContainerDivHeight + 25);
            $(dataContainerDiv.parentElement).css("height", 130 + dataContainerDivHeight);//100是给表头预留的位置
        },
