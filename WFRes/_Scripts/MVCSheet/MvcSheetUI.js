...
// var formUrl='http://192.168.17.42:8080/stzx/';
// var formUrl='http://180.168.218.158:8010/stzx/';

var forurlpost = window.location.host;
var formUrl =  'http://'+forurlpost .substring(0,forurlpost.indexOf(':'))+':8080/stzx/';
$('div').scroll(function () {
    $("#main-navbar").css({ "top": $(this).scrollTop()});
});
// var URLNew= 'http://'+urlpost .substring(0,urlpost.indexOf(':'))+':8080/stzx/portal/';
// var formUrl='http://taizhou.iask.in:26753/stzx/';
//// 同步方式获取本地化字符串
//$.ajax({
//    type: "GET",
//    url: _PORTALROOT_GLOBAL + "/Ajax/GlobalHandler.ashx?Code=RegularExpression_errorMessage",
//    async: false,
//    dataType: "json",
//    success: function (data) {
//        if (data.IsSuccess) {
//            MvcSheetUIGlobalString = data.TextObj;
//        }
//    }
//});
//组件基类
(function ($) {
                ...
                if (datafiled.indexOf("#") == 0) {
                    ...
                    if (datafiled.indexOf('.') > -1) {
                        ...
                        $("[" + this.PreDataKey + this.DataFieldKey.toLowerCase() + "='" + datafiled + "']").each(function () {
                            ...
                            //else if (datafiled.indexOf('.') > -1) {
                            //    //子表数据项
                            //    if (rownum == undefined) {
                            //        row = 1;//默认第一行
                            //    }
                            //    //找到对应的行
                            //    //Error:子表都得通过子表管理器，去找到对应行的元素
                            //    if ($.MvcSheetUI.QueryString("ismobile") == "true") {
                            //        //if ($(this).closest("ul[data-row='" + row + "']").length == 0) {
                            //        //    flag = false;
                            //        //}
                            //        var gridview = $.MvcSheetUI.GetElement(datafiled.split('.')[0]);// $("div[" + this.PreDataKey + this.DataFieldKey.toLowerCase() + "='" + datafiled.indexOf('.')[0] + "']");
                            //        var sheetGridView = gridview.SheetGridView();
                            //        if (sheetGridView) {
                            //            for (var e = 0; e < sheetGridView.MobileRows[row - 1].Editors.length; e++) {
                            //                editor = sheetGridView.MobileRows[row - 1].Editors[e];
                            //                if (editor.DataField == datafiled) {
                            ...
                            //        }
                            //    }
                            //    else if ($(this).closest("tr[data-row='" + row + "']").length == 0) {
                            //        flag = false;
                            //    }
                            //}
                            if (flag) {
            ...
            Validate: function (actionControl, effective) {
                ...
                if (!flag) {
                    ...
                    if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                        // layer.open({
                        //     content: this.ValidateText
                        //     ,btn: '我知道了'
                        // });
                        alert(this.ValidateText);
                    }else {
                        $('#message-module').find('.modal-body').html('<h4 style="text-align: center;padding: 50px;">'+this.ValidateText+'</h4>');
        ...
        LoadingMask: {
            Show: function (msg, isAlpha) {
                if ($.MvcSheetUI.QueryString("ismobile")
                    ...
                    //添加消息
                    this.MsgDiv = $("<div></div>");
                    this.MsgDiv.append($("<img src='" + $.MvcSheetUI.PortalRoot + "/WFRes/_Content/themes/ligerUI/Aqua/images/common/bigloading.gif' />"));
    ...
    $.MvcSheetUI.IControl = function (element, options, sheetInfo) {
        //是否支持Html5
        this.IsHtml5 = window.applicationCache != undefined;
        ...
        // 逻辑类型
        this.LogicType = this.L || $.MvcSheetUI.LogicType.ShortString;
        // 是否可见
        ...
        if (this.IsMobile) {
            ...
            if ($(this.Element).parent().length != 0 && ($(this.Element).parent().attr("class").indexOf("col-md-4") > -1 || $(this.Element).parent().attr("class").indexOf("col-md-10") > -1)) {
                ...
                div.before($("<span class='input-label'>" + parent.data("title") + "</span>"));

                if (parent.prev().attr("class").indexOf("col-md-2") > -1) { parent.prev().remove() }
            }
    ...
    $.MvcSheetUI.IControl.prototype = {
        ...
        MoveToMobileContainer: function (_Editor) {
            ...
            if (this instanceof $.MvcSheetUI.Controls.SheetTimeSpan ||
                this instanceof $.MvcSheetUI.Controls.SheetUser) {
                _Element.remove();
            } else {
                //Error:这个逻辑需要改一下
        ...
        RenderDataTrackLink: function () {
            ...
            var html = "<a class=\"nav-icon fa fa-list-alt\" style=\"padding-left:5px\" data-toggle=\"modal\" data-target='#" + popupWindowId + "'></a>";
            $(html).insertAfter($(this.Element));

            var url = $.MvcSheetUI.PortalRoot + "/InstanceDataTrack.html?";
            url += "InstanceId=" + $.MvcSheetUI.SheetInfo.InstanceId + "&ItemName=" + this.DataField + "&F=frm_" + popupWindowId;
            this.CreatePopupDiv(popupWindowId, "修改痕迹", url, "400px");
        },
