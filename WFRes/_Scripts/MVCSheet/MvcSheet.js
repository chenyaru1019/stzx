...
jQuery.extend({
    MvcSheet: {
        ...
        ForwardAction: { Action: "Forward", Icon: "fa-mail-forward", Text: "转发", en_us: "Forward" },
...
jQuery.extend(
    $.MvcSheet,
    {
            ...
            if ($.MvcSheetUI.SheetInfo.IsMobile) {
                ...
                if (_Actions.length == 0) {
                    ...
                    _Actions.forEach(function (e) {
                        ...
                        if (e.actionKey == "Save") { strClass = "icon-left ion-android-archive"; }
                        if (e.actionKey == "Submit") { strClass = "icon-left ion-checkmark-round"; }
                        var button = $("<div class=\"button foot-button " + strClass + " button-clear \"></div>")
                        button.text(e.text);
                    ...
                    var moreActions = [];
                    _Actions.forEach(function (e) {
                        if (e.actionKey == "Save" || e.actionKey == "Submit") {
                            var strClass = "icon-left ion-checkmark-round";
                            if (e.actionKey == "Save") { strClass = "icon-left ion-android-archive"; }
                            var button = $("<div class=\"button foot-button " + strClass + " button-clear \"></div>")
                            button.text(e.text);
                            button.unbind("click." + e.actionKey).bind("click." + e.actionKey, function () { manager.ControlManagers[e.actionKey].ActionClick(); })
                    ...
                    if (moreActions.length > 0) {
                        var btnMore = $("<div class=\"button foot-button button-clear\">更多...</div>")
                        var buttonsA = [];
            ...
            this.GetSheet(
                ...
                function (data) {
                    ...
                    if ($.MvcSheetUI.SheetInfo.IsMobile) {
                        ...
                        // $('#WorkDaily').css('display','none');
                        $('#WorkDaily').addClass('bannerTitle');
                        //是否找到默认审批控件
                ...
                if (selects && selects.length > 0) {
                    for (var i = 0; i < selects.length; i++) {
                        ...
                        if (manager && !manager.Editable) {
                            if ($.MvcSheetUI.SheetInfo.IsOriginateMode && $(selects[i]).attr("data-datafield") == "Originator.OUName" && $(selects[i]).find("option").length > 1) return;
        ...
        Action: function (actionControl) {
            ...
            //console.log(actionName + "：Done...");
        },

        ConfirmAction: function (message, doneCallback) {
            if ($.MvcSheetUI.IonicFramework.$ionicPopup) {
                showConfirm = function () {
                    var confirmPopup = $.MvcSheetUI.IonicFramework.$ionicPopup.confirm({
                        title: SheetLanguages.Current.Notice,
                        template: message,
                        buttons: [
                ...
                console.log(message);
                var moduleFooter = $('#messageModule').find('.modal-footer');
                ...
                $('#messageModule').find('.modal-body').html('<h4 style="text-align: center;padding: 50px;">'+message+'</h4>');
                $('#messageModule').modal({keyboard: true, show: true, backdrop: true,});

                confirmButton.click(function() {
                    $('#messageModule').modal('hide');
                    doneCallback();
                });
            }
        },
        ...
        Save: function (actionControl) {
            ...
            // $('#message-module').find('.modal-body').html('<h4 style="text-align: center;padding: 50px;">保存成功</h4>');
            ...
            this.PostSheet(
                    ...
                    function (data) {
                        if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){
                            ...
                            $.LoadingMask.Hide();
                            $('#message-module').find('.modal-body').html('<h4 style="text-align: center;padding: 50px;line-height: 25px;">保存成功</h4>');
                            $('#message-module').modal({keyboard: true, show: true, backdrop: true});

                            $('#message-module').on('hidden.bs.modal', function () {
                                that.ResultHandler.apply(that, [actionControl, data]);
                            });
                        }
                    ...
                    function (data) {
                        ...
                        if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){
                            ...
                            if (data.Successful) {
                                ...
                                //让弹出框显示的操作方法
                                $('#message-module').find('.modal-body').html('<h4 style="text-align: center;padding: 50px;line-height: 25px">提交成功</h4>');
                                $('#message-module').modal({keyboard: true, show: true, backdrop: true});
                                ...
                                $('#message-module').find('.modal-body').html('<h4 style="text-align: center;padding: 50px;">提交失败</h4>');
                                $('#message-module').modal({keyboard: true, show: true, backdrop: true});
                                $('#message-module').on('shown.bs.modal', function() {
                                    $.LoadingMask.Hide();
                                });
        ...
        Reject: function (actionControl, destActivity) {
            ...
            var SheetPostValue = this.GetMvcPostValue(this.Action_Reject, destActivity);
            var that = this;
            this.PostSheet(
                {
                    Command: this.Action_Reject,
                    MvcPostValue: JSON.stringify(SheetPostValue)
                },
                function (data) {
                    that.ResultHandler.apply(that, [actionControl, data]);
        ...
        ClosePage: function () {
            ...
            try {
                ...
                if (!this.IsPC()) {
                    console.log('我是移动端啊');
                    window.open('/Hybrid/index.html#/home/', '_self');
                }
            } catch (e) {
                window.close();
            }
