...
namespace OThinker.H3.Portal
{
    ...
    public class m : System.Web.Services.WebService
    {
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public void GetWorkItems(string userId, string mobileToken, string keyWord,
                DateTime lastTime,
                string sortKey,
                string sortDirection,
                bool finishedWorkItem, DateTime startDate, DateTime endDate)
        {
            int returnCount = 10;//返回数量，默认为10；
            if (!ValidateMobileToken(userId, mobileToken)) return;
            MobileAccess mobile = new MobileAccess();
            var Items = mobile.GetLoadWorkItems(userId, mobileToken, keyWord, lastTime, sortKey, sortDirection, finishedWorkItem, Instance.PriorityType.Unspecified, startDate, endDate, returnCount);
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public void GetWorkItemsByPriority(string userId, string mobileToken, string keyWord,
                DateTime lastTime,
                string sortKey,
                string sortDirection,
                bool finishedWorkItem)
        {
            ...
            var HighPriorityItems = mobile.GetLoadWorkItems(userId, mobileToken, keyWord, lastTime, sortKey, sortDirection, finishedWorkItem, Instance.PriorityType.High, DateTime.Parse("2004-01-01"), DateTime.MaxValue, returnCount);
            ...
            {
                returnCount = 10 - HighPriorityItems.WorkItems.Count;
            }

            if (showNormal)
            {
                NormalPriorityItems = mobile.GetLoadWorkItems(userId, mobileToken, keyWord, lastTime, sortKey, sortDirection, finishedWorkItem, Instance.PriorityType.Normal, DateTime.Parse("2004-01-01"), DateTime.MaxValue, returnCount);
            }
        ...
        public void GetRefreshWorkItems(string userId, string mobileToken, string keyword, DateTime LastTime,
            bool finishedWorkItem, int existsLength, DateTime startDate, DateTime endDate)
        {
            int returnCount = 10;//返回数量，默认为10；

            MobileAccess mobile = new MobileAccess();
            var Items = mobile.GetRefreshWorkItemsFn(userId, mobileToken, keyword, LastTime, finishedWorkItem, existsLength, Instance.PriorityType.Unspecified, startDate, endDate, returnCount);
            ResponseJSON(JSSerializer.Serialize(Items));
            ...
            bool readWorkItem)
        {
            ...
            if (!ValidateMobileToken(userId, mobileToken)) return;
            MobileAccess mobile = new MobileAccess();
            var result = mobile.LoadCirculateItemsFn(userId, mobileToken, Server.UrlDecode(keyWord), lastTime, sortKey, sortDirection, readWorkItem);
            if (result == null) return;
            ResponseJSON(JSSerializer.Serialize(result));
        ...
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public void LoadAllInstances(string userId, string mobileToken)
        {
            userId = AppUtility.RegParam(userId);
            if (!ValidateMobileToken(userId, mobileToken)) return;
            MobileAccess mobileAccess = new MobileAccess();
            DateTime lastTime = DateTime.Now;
            var result = mobileAccess.LoadAllInstances(userId, lastTime);
            ResponseJSON(JSSerializer.Serialize(result));
        }

        [WebMethod(Description = "获取指定状态的流程实例")]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
        public void LoadInstancesByState(string userId, string mobileToken, string keyWord, int instanceState, DateTime lastTime)
        {
            userId = AppUtility.RegParam(userId);
            if (!ValidateMobileToken(userId, mobileToken)) return;
            MobileAccess mobileAccess = new MobileAccess();
            var result = mobileAccess.LoadInstances(userId, Server.UrlDecode(keyWord), instanceState, lastTime);
            ResponseJSON(JSSerializer.Serialize(result));
        }
