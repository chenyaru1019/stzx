        ...
        if (typeof (InstanceContext) != "undefined" && InstanceContext) {
            ...
            $(workflow.activities).each(function () {
                ...
                if (typeof (ExceptionActivities) != "undefined" && $(ExceptionActivities).filter(function () {
                    ...
                    $(InstanceContext.Tokens).each(function () {
                        if (this.Activity == _thisActivity.ActivityCode) {
                            //只有流程在进行中才显示进行中的任务
                            if (!InstanceExceptional && (InstanceContext.State == InstanceState.Running || InstanceContext.State == "Running") && (this.State == TokenState.Running || this.State == "Running"))
                                _ExistUnfinished = true;
                                //已完成
                ...
                if (_PlayAble && $("#btnReplay").length == 0 && $("#lblWorkflowTitle").length == 1) {
                    ...
                    $("#lblWorkflowTitle").append('<span>[</span><a id="btnReplay" style="cursor: pointer;">点击流程回放</a><span>]</span>');
                ...
                if ($("#btnImage").length == 0 && window.applicationCache) {
                    ...
                    $("#lblWorkflowTitle").append('<span>[</span><a id="btnImage" style="cursor: pointer;">存为图片</a><span>]</span>');
                    var ImageButton = $("#btnImage");
                    ImageButton.click(function (e) {
                        e.stopPropagation();
                        var s = 0;
                        s = setInterval(function () {
                            var base64 = $('#divImage').find("img").attr("src");
                            if (base64 != undefined) {
                                clearInterval(s)
