...
; (function ($) {
    ...
    $.MvcSheetUI.Controls.SheetDropDownList.Inherit($.MvcSheetUI.IControl, {
        ...
        Render: function () {
            ...
            var that = this;
            //是否可见
            if (!this.Visiable) { $element.hide(); return; }
            ...
            if (this.SchemaCode.trim() != "") { //业务对象数据源（主数据）
                ...
                if ($.MvcSheetUI.CacheData && $.MvcSheetUI.CacheData[cmdParam]) {
                    ...
                    for (var key in cacheData) {
                        ...
                        if (this.IsMobile) {
                            this.AddMobileItem(key, data[key], false);
                        }
                    }
                            ...
                            if (data) {
                                ...
                                for (var i = 0, len = data.length; i < len; i++) {
                                    $element.append("<option value=\"" + data[i].Code + "\"" +
                                        (data[i].IsDefault ? " selected=\"selected\"" : "") + ">" + data[i].EnumValue + "</option>");
                                    if (that.IsMobile) {
        ...
        ionicInit: function (ionic) {
            ...
            $(this.Element.parentElement.parentElement).unbind('click.showChoice').bind('click.showChoice', function (e) {
                ...
                var templateurl = $.MvcSheetUI.PortalRoot + '/Hybrid/form/templates/dropDownList.html';
                ionic.$scope.DropDownListDisplay = that.MobileOptions;
                ionic.$scope.DropDownListValue = that.GetValue();
                ionic.$scope.clickRadio = function (value, text) {
                    that.SetValue(value);
                    $(that.Mask).html(text);
                    myPopup.close();
                }
                var myPopup = ionic.$ionicPopup.show({
                    title: title,
                    templateUrl: templateurl,
                    scope: ionic.$scope
                });
                var htmlEl = $('html');
                htmlEl.unbind("click.close").bind('click.close', function (event) {
        ...
        _selectItem: function ($element, selectedValue) {
            ...
            if (!this.Editable) {
                ...
                if (textLable.length > 0) {
                    ...
                    $element.after("<label id='myselecttext' style='width:100%;'>" + this.GetText() + "</label>");
                }
            }
