...
namespace OThinker.H3.DingTalk
{
    ...
    public class DingTalkService : System.Web.Services.WebService
    {
        ...
        public static string DDUnFinishedWork = "http://140.206.138.190:8010/Portal/Hybrid/index.html?target=home.index&state=DefaultEngine&loginfrom=dingtalk";
        ...
        //待办提醒
        [WebMethod]
        public string SendNoticeByActivityCode2(string ActivityCode, string InstanceID)
        {
            //判断
            if (string.IsNullOrEmpty(ActivityCode))
                return "ActivityCode不能为空";
        ...
        //子流程待办提醒（根据父流程id查询子流程第一个节点）
        [WebMethod]
        public string SendChildNoticeByActivityCode(string InstanceID,string ActivityCode,string Title)
        {
            //判断
            if (string.IsNullOrEmpty(ActivityCode))
                return "ActivityCode不能为空";
            ...
            string msg = "";
            //取当前节点的所有任务
            string sql = "SELECT wi.ObjectID, wi.Participant,wi.TokenId," +
                " wi.Delegant,wa.WorkflowName FROM OT_WorkItem wi " +
                " INNER JOIN OT_WorkflowClause wa ON wi.WorkflowCode = wa.WorkflowCode " +
                " WHERE wi.InstanceId IN ( SELECT ObjectID FROM OT_InstanceContext " +
                " WHERE ParentInstanceID = '"+InstanceID+"'" +
                " AND ParentActivityCode = 'Activity35') AND wi.ActivityCode = 'Activity2'";
            DataTable dtworkitem = OThinker.H3.Controllers.AppUtility.Engine.EngineConfig.CommandFactory.CreateCommand().ExecuteDataTable(sql);
            foreach (DataRow item in dtworkitem.Rows)
            {
                Title = "待办通知:" + Title;
                Body = "您有1件新的" + Title+ "需要审核，请及时处理！";
