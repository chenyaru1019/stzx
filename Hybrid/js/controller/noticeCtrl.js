module.controller('afficheCtrl',
    function ($rootScope, $scope, $http, $state, $ionicLoading, $ionicScrollDelegate, $ionicModal, $window, $ionicActionSheet, commonJS, focus) {
        ...
        $scope.openCirculateItem = function (workItemId) {
            ...
            $scope.worksheetUrl = $scope.setting.workItemUrl.split('portal')[0] +'Portal/Sheets/stzx/NewNotice/NewNoticeMobile.aspx?SheetCode=SViewNotice&IsMobile=True&Mode=Work&WorkItemID='+workItemId+'&LoginSID='+$scope.clientInfo.UUID+'&MobileToken'+ $scope.user.MobileToken;
            commonJS.openWorkItem($scope, $scope.worksheetUrl, workItemId);
            // var url = $scope.setting.workItemUrl.split('portal')[0] +'Portal/Sheets/stzx/NewNotice/NewNoticeMobile.aspx?SheetCode=SViewNotice&IsMobile=True&Mode=Work&WorkItemID='+workItemId+'&T='+new Date().getTime()+'&loginfrom=wechat#/form/detail';
        }
...
module.controller('SearchNoticeWorkItemCtr', function ($scope, $http, $ionicPopup, $ionicModal, $ionicActionSheet, $ionicScrollDelegate, $ionicTabsDelegate, commonJS, focus) {
    //进行搜索
    $scope.noticeViewItems_Search = [];
    ...
    $scope.loadMoreWorkItem = function () {
        ...
        commonJS.getHttpData(url, params)
            .success(function (result) {
                if (result.NewsList && result.NewsList.length > 0) {
                    for (var i = 0; i < result.NewsList.length; i++) {
                        if ($scope.existsWorkItem_Search($scope.noticeViewItems_Search, result.NewsList[i])) continue;
                        $scope.noticeViewItems_Search.push(result.NewsList[i]);
                    }
                }
                $scope.initCompleted = true;
                $scope.exception = false;
