formModule.factory('sheetUserService', function (httpService) {
    return {
        ...
        checkItems: function (users, items) {
            if (!users || users.length == 0 || items.length == 0) return users;
        ...
        initItems: function (users) {
            ...
            if (users) {
                if (users.constructor == Object) {
                    var tempUser = { id: users.ObjectID, name: users.Name };
                    objs.push(tempUser);
                } else {
                    users.forEach(function (n, i) {
                        var tempUser = { id: n.ObjectID, name: n.Name };
                        objs.push(tempUser);
                    });
                }
            }
        ...
        convertItems: function (items) {
            if (!items) return [];
            if (items.length == 1) {
                var obj = { Code: items[0].id, Name: items[0].name };
                return obj;
            } else {
                var objs = [];
                items.forEach(function (n, i) {
                    objs.push({ Code: n.id, Name: n.name });
                });
                return objs;
            }
        }
