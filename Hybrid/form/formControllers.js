...
.controller("mainCtrl", function ($rootScope, $scope, $state, $compile, $http, $ionicPopup, $timeout, fcommonJS) {
    $rootScope.dingMobile = {
        isDingMobile: true,                             //是否钉钉移动端，如果是钉钉移动端，需要隐藏当前header，重写钉钉APP Header
...
.controller("formSheetCtrl", function ($rootScope, $scope, $state, $timeout, $compile, $http, $ionicPopup, $ionicPlatform, $ionicTabsDelegate, $cordovaDevice, $cordovaAppVersion, $cordovaNetwork, $ionicScrollDelegate, $ionicActionSheet, $ionicHistory, fcommonJS) {
    //PC端html内容
    ...
    $scope.ClosePage = function () {
        ...
        if ($rootScope.dingMobile.isDingMobile) {
        ...
        else if (window.parent && window.parent._scope && window.parent._scope.modalForm) {
            console.log('钉钉显示');
            //微信
            ...
            //app
            console.log('钉钉app关闭按钮');
            window.open("/Hybrid/index.html/home");
 ...
 .controller('sheetUserCtrl', function ($rootScope, $scope, $ionicActionSheet, $state, $stateParams, $ionicBackdrop, $timeout, $ionicModal, $ionicSideMenuDelegate, $ionicHistory, sheetUserService) {
     ...
     $scope.SelectStructure = function (SelectFormStructure) {
         ...
         $scope.showDeptID = $.MvcSheetUI.SheetInfo.Company.ObjectID;
         $scope.deptNav.push({
             id: "",//$.MvcSheetUI.SheetInfo.Company.ObjectID,
             name: $.MvcSheetUI.SheetInfo.Company.Name,
             pid: "",
             index: "0"
         });
         //加载数据
     ...
     $scope.itemClick = function (e, org) {
         ...
         if (org.type != "U" && !$scope.SearchMode && (!org.canSelect || Percent > 75)) {
             //展开部门  
         ...
         sheetUserService.loadData($scope.sheetUserHandler + "?ParentID=" + parentid + "&isMobile=true&" + querystr, null).then(function (res) {
             ...
             if (parentid == "" && $scope.sheetUserParams.options.RootSelectable && $scope.sheetUserParams.options.OrgUnitVisible == "true") {
                 ...
                 var root = {
                     ...
                     canSelect: true,
                     checked: false,
                     code: $.MvcSheetUI.SheetInfo.Company.ObjectID,
                     id: $.MvcSheetUI.SheetInfo.Company.ObjectID,
                     name: $.MvcSheetUI.SheetInfo.Company.Name,
                     type: "U"
                 };
...
.controller('downLoadFileCtrl', ['$rootScope', '$scope', '$location', '$http', '$stateParams', '$ionicHistory', '$window', function ($rootScope, $scope, $location, $http, $stateParams, $ionicHistory, $window) {
    $scope.$on("$ionicView.enter", function (scopes, states) {
        ...
        if ($rootScope.dingMobile.isDingMobile) {
            ...
            if ($stateParams.isimg == "true") {
                var a = angular.element(document.querySelector("#img")).attr("src", $stateParams.url);
            } else {
