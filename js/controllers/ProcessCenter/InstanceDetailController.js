app.controller('InstanceDetailController', ['$rootScope', '$scope', '$state', '$stateParams', '$http', '$compile', '$modal', '$window', '$translate', '$timeout', 'ControllerConfig',
    function ($rootScope, $scope, $state, $stateParams, $http, $compile, $modal, $window, $translate, $timeout, ControllerConfig) {
            ...
            $http({
                ...
                .success(function (result, header, config, status) {
                    if (result.Success) {
                        ...
                        for (var i = 0; i< result.Extend.InstanceLogInfo.length; i++) {
                            if (result.Extend.InstanceLogInfo[i].ChildInstanceId != "") {
                                $.ajax({
                                    type: "POST",
                                    url: "/Portal/Sheets/handlers/CommonHandler.ashx?Command=GetNameById",   //处理页的相对地址
                                    data: {
                                        UserId: result.Extend.InstanceLogInfo[i].Participant
                                    },
                                    async: false,
                                    success: function (e) {
                                        if (e != null && e != '') {
                                            result.Extend.InstanceLogInfo[i].ParticipantName = e;
                                        }
        ...
        $scope.DoMerge = function () {
            var ths = angular.element($('#TokenGrid tbody .status').find('.font_color'));
            // console.log(ths)
            angular.forEach(ths, function (data, index, full) {
                console.log($(data).html());
                if ($(data).html() == '通过'|| $(data).html() == '已结束') {
                    $(data).css('color', '#1ABC9C');
                }
                if ($(data).html() == '处理中' || $(data).html() == '等待中' || $(data).html() == '运行中') {
                    $(data).css('color', '#FFC81D');
                }
                if ($(data).html() == '拒绝') {
                    $(data).css('color', 'red');
                }
    ...
    $scope.ok = function () {
        ...
        $http({
            ...
            .success(function (data, header, config, status) {
                $.ajax({
                    type: "POST",
                    url: "/Portal/Sheets/handlers/DingTalkHandler.ashx?Command=SendNoticeByInstanceId",   //处理页的相对地址
                    data: {
                        Userid: "18252688571",
                        Tel: "",
                        Instanceid:params.InstanceID,
                        Title: "催办提醒",
                        Body: $scope.UrgeContent
                    },
                    async: false,
                    success: function (e) {
                        console.log(e)
                ...
                $modalInstance.dismiss('cancel'); // 退出
                $.notify({ message: $translate.instant("InstanceDetail.UrgedSuccess"), status: "danger" });
            })
            .error(function (data, header, config, status) {
                $modalInstance.dismiss('cancel'); // 退出
            })
