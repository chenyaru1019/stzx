...
app.controller('BizQueryViewController', ['$rootScope', '$scope', '$http', '$state', '$stateParams', '$translate', '$modal', '$compile', '$timeout', '$filter', '$interval', 'jq.datables', 'datecalculation', 'ControllerConfig',
    function ($rootScope, $scope, $http, $state, $stateParams, $translate, $modal, $compile, $timeout, $filter, $interval, jqdatables, datecalculation, Controller) {
        ...
        $scope.url =  'http://'+urlpost .substring(0,urlpost.indexOf(':'))+':8080/stzx/';

        // $scope.url='http://192.168.17.42:8080/stzx/';
        // $scope.url='http://140.206.138.190:5023/stzx/';
        ...
        $scope.init = function () {
            ...
            $.ajax({
                ...
                success: function (data, header, config, status) {
                    ...
                    //移除查询类页面选择按钮
                    if ($scope.QueryCode == 'ViewNews' || $scope.QueryCode == 'ggcx' || $scope.QueryCode == 'wlzycx'  || $scope.QueryCode == 'FAQuery') {
                        $scope.GirdColumns.splice(0, 1);
                    }
                    if ($scope.QueryCode == 'getConsumables' || $scope.QueryCode == 'sbxxhcx' || $scope.QueryCode == 'TXLCX' || $scope.QueryCode == 'yzcx') {
                        $scope.GirdColumns.splice(0, 1);
                    }
                    if ($scope.QueryCode == 'ddd' || $scope.QueryCode == 'lczncx' || $scope.QueryCode == 'zjxxcx' || $scope.QueryCode == 'zlzxcx') {
                        $scope.GirdColumns.splice(0, 1);
                    }
                    if ($scope.QueryCode == 'xmxxcx' || $scope.QueryCode == 'QueryTemplate') {
                        $scope.GirdColumns.splice(0, 1);
                    }
                    if ($scope.QueryCode == 'qkcx' || $scope.QueryCode == 'dtxwcx') {
                        $scope.GirdColumns.splice(0, 1);
                    }
                    //项目档案管理
                    if ($scope.QueryCode == 'dakcx' || $scope.QueryCode == 'dakwh' || $scope.QueryCode == 'jttxlcx') {
                        $scope.GirdColumns.splice(0, 1);
                    }
        ...
        $scope.BizViewOptions = function () {
            var options = {
                ...
                "fnServerData": function (sSource, aDataSet, fnCallback) {
                    $.ajax({
                        ...
                        "success": function (json) {
                            ...
                            //新闻、公告发布日期时间排序
                            if($scope.QueryCode==='xwylgl' || $scope.QueryCode==='gggl'){
                                var rows = json.Rows;
                            ...
                            //地铁新闻、期刊时间去时分秒
                            if($scope.QueryCode==='qkcx' || $scope.QueryCode==='qkwh' || $scope.QueryCode==='dtxwcx' || $scope.QueryCode==='dtxwwh'){
                                var rows = json.Rows;
                                for (var i = 0;i<rows.length;i++){
                                    rows[i].PublishTime = rows[i].PublishTime.split(' ')[0];
                                }
                                fnCallback(json);
                            }
                            fnCallback(json);
                ...
                "fnCreatedRow": function(nRow, aData, iDataIndex ) {
                    //咨询公司公告查阅
                    if ($scope.QueryCode == 'ggcx' && $scope.sheetCode == undefined) {
                        var url = '';
                    ...
                    //咨询公司新闻查阅
                    if ($scope.QueryCode == 'ViewNews' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function() {
                    ...
                    //公文库添加链接
                    if ($scope.QueryCode==='ddd' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function() {
                            url = '/Portal/Sheets/DefaultEngine/DocumentInfo/SDocumentInfo.aspx?SheetCode=SDocumentInfo&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src',url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //资料中心查询添加链接
                    if ($scope.QueryCode==='zlzxcx' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function() {
                            url = '/Portal/MvcDefaultSheet.aspx?SheetCode=SMaterialCenter&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src',url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //办事指南添加链接
                    if ($scope.QueryCode==='lczncx' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function() {
                            url = '/Portal/MvcDefaultSheet.aspx?SheetCode=SFlowForm&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src',url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //专家库查询添加链接
                    if($scope.QueryCode==='zjxxcx' && $scope.sheetCode == undefined){
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function() {
                            url = '/Portal/Sheets/DefaultEngine/ExpertInfo/SExpertInfo.aspx?SheetCode=SExpertInfo&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src',url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //项目信息查询添加链接
                    if($scope.QueryCode==='xmxxcx' && $scope.sheetCode == undefined){
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function() {
                            url = '/Portal/Sheets/stzx/ProjectInfo/SProjectInfo.aspx?SheetCode=ProjectInfo&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src',url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    //项目档案库查询
                    if ($scope.QueryCode === 'dakcx' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function () {

                            url = '/Portal/Sheets/stzx/ArchiveInfo/SArchiveInfo.aspx?Mode=Originate&SchemaCode=ArchiveInfo&Flag=0&ProjectId=' + aData.ProjectId;
                            $(".app-aside-right").find('iframe').attr('src', url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //项目档案库维护
                    if ($scope.QueryCode === 'dakwh' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function () {
                            url = '/Portal/Sheets/stzx/ArchiveInfo/SArchiveInfo.aspx?Mode=Originate&SchemaCode=ArchiveInfo&Flag=1&ProjectId=' + aData.ProjectId;
                            $(".app-aside-right").find('iframe').attr('src', url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //Office模板查询添加链接
                    if ($scope.QueryCode === 'QueryTemplate' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function () {
                            url = '/Portal/Sheets/stzx/OfficeTemplate/SOfficeTemplate.aspx?SheetCode=SOfficeTemplate&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src', url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //集团通讯录维护添加链接
                    if ($scope.QueryCode === 'jttxlcx' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function () {
                            url = '/Portal/MvcDefaultSheet.aspx?SheetCode=SGroupAddressbook&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src', url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //地铁期刊加链接
                    if ($scope.QueryCode === 'qkcx' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function () {
                            url = '/Portal/Sheets/stzx/SubwayJournals/SSubwayJournals.aspx?SheetCode=SSubwayJournals&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src', url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                    ...
                    //地铁新闻添加链接
                    if ($scope.QueryCode === 'dtxwcx' && $scope.sheetCode == undefined) {
                        var url = '';
                        $(nRow).css('cursor', 'pointer').click(function () {
                            url = '/Portal/Sheets/stzx/SubwayNews/SSubwayNews.aspx?SheetCode=SSubwayNews&Mode=View&SchemaCode=' +$scope.SchemaCode+ '&BizObjectID=' + aData.ObjectID;
                            $(".app-aside-right").find('iframe').attr('src', url);
                            $('.app-aside-right').addClass('show');
                        });
                    }
                ...
                "initComplete": function (settings, json) {
                    ...
                    $('.filter-zjxxcx').css('display', 'block').appendTo($('.query-zjxxcx'));
                    $('.filter-ddd').css('display', 'block').appendTo($('.query-ddd'));
                    $('.filter-xmxxcx').css('display', 'block').appendTo($('.query-xmxxcx'));
                    $('.filter-dakcx').css('display', 'block').appendTo($('.query-dakcx'));
                    $('.filter-dakwh').css('display', 'block').appendTo($('.query-dakwh'));
                    ...
                    $('.queryCriteria-zjxxcx').css('display','block');
                    $('.queryCriteria-ddd').css('display','block');
                    $('.queryCriteria-xmxxcx').css('display','block');
                    $('.queryCriteria-dakcx').css('display','block');
                    $('.queryCriteria-dakwh').css('display','block');
                    ...
                    $('#Btn_dropDown-zjxxcx').css('display','block');
                    $('#Btn_dropDown-ddd').css('display','block');
                    $('#Btn_dropDown-xmxxcx').css('display','block');
                    $('#Btn_dropDown-dakcx').css('display','block');
                    $('#Btn_dropDown-dakwh').css('display','block');
                    ...
                    $('#Btn_dropDown-zjxxcx').on('click',function () {
                        $('.query-zjxxcx').toggle(200);
                        $('.Btn_dropDown').toggleClass('Btn_dropup');
                    });
                    $('#Btn_dropDown-ddd').on('click',function () {
                        $('.query-ddd').toggle(200);
                        $('.Btn_dropDown').toggleClass('Btn_dropup');
                    });
                    $('#Btn_dropDown-xmxxcx').on('click',function () {
                        $('.query-xmxxcx').toggle(200);
                        $('.Btn_dropDown').toggleClass('Btn_dropup');
                    });
                    $('#Btn_dropDown-dakcx').on('click',function () {
                        $('.query-dakcx').toggle(200);
                        $('.Btn_dropDown').toggleClass('Btn_dropup');
                    });
                    $('#Btn_dropDown-dakwh').on('click',function () {
                        $('.query-dakwh').toggle(200);
                        $('.Btn_dropDown').toggleClass('Btn_dropup');
                    });
                    //网络资源台账维护添加 导入导出按钮
                    if($scope.QueryCode=='wlzygl') {
                    ...
                    if($scope.QueryCode=='TXLCX') {
                        var importBtn = '<div style="padding: 0 10px;text-align: right;margin-top: 15px;"><a class="btn btn-default" href="'+url+'ExportExcel/exportaddressBook">导出通讯录</a></div>';
                        $(importBtn).prependTo($('.table-TXLCX'));
                    }
